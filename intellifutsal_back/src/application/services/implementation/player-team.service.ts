...
import { CreatePlayerTeamRequest, OnboardingStatus, PlayerTeamResponse, UpdatePlayerTeamRequest, UpdateStatusRequest } from "../../../domain/interfaces";
import { PlayerTeamMapper } from "../../mappers";
import { Player, PlayerTeam, Team } from "../../../infrastructure/models";
import { CredentialRepository, JoinRequestRepository, PlayerRepository, PlayerTeamRepository, TeamRepository } from "../../../infrastructure/repository";
import { CREDENTIAL_NOT_FOUND, INTERNAL_SERVER_ERROR, PLAYER_NOT_FOUND, PLAYER_TEAM_ALREADY_ASSOCIATED, PLAYER_TEAM_ENTRY_DATE_FUTURE_ERROR, PLAYER_TEAM_EXIT_DATE_BEFORE_ENTRY_ERROR, PLAYER_TEAM_EXIT_DATE_FUTURE_ERROR, PLAYER_TEAM_NOT_FOUND, PLAYER_TEAM_SAVE_ERROR, PLAYER_TEAM_UPDATE_ERROR, TEAM_NOT_FOUND } from "../../../core/utilities/messages.utility";
...
export class PlayerTeamService implements IPlayerTeamService {
    ...
    public save = async (createPlayerTeamRequest: CreatePlayerTeamRequest): Promise<PlayerTeamResponse> => {
        ...
        await this.recomputePlayerOnboardingStatus(player.id);
    ...
    private findCredentialIdOrThrow = async (playerId: number): Promise<number> => {
        ...
        if (!credentialId) throw new NotFoundException(`${ CREDENTIAL_NOT_FOUND }${ playerId }`);
    ...
    private recomputePlayerOnboardingStatus = async (playerId: number): Promise<void> => {
        ...
        const isProfileComplete = this.isPlayerProfileComplete(player);
        if (!isProfileComplete) {
            await this.credentialRepository.updateOnboardingStatus(credentialId, OnboardingStatus.PROFILE_INCOMPLETE);
            return;
        }
        ...
        const newStatus = hasPendingRequests ? OnboardingStatus.TEAM_PENDING : OnboardingStatus.TEAM_PENDING;

        await this.credentialRepository.updateOnboardingStatus(credentialId, OnboardingStatus.TEAM_PENDING);
    };
}