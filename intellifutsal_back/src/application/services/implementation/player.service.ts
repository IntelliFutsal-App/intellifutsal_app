...
import { ConflictException, InternalServerException, NotFoundException } from "../../../domain/exceptions";
import { CreatePlayerRequest, OnboardingStatus, PlayerResponse, Role, UpdatePlayerRequest, UpdateStatusRequest } from "../../../domain/interfaces";
import { PlayerMapper } from "../../mappers";
import { Credential, Player } from "../../../infrastructure/models";
import { CoachRepository, CredentialRepository, PlayerRepository, PlayerTeamRepository } from "../../../infrastructure/repository";
import { LOGIN_URL } from "../../../core/utilities/constants.utility";
import { CREDENTIAL_ALREADY_ASSIGNED_PLAYER, CREDENTIAL_ALREADY_ASSIGNED_COACH, CREDENTIAL_NOT_FOUND, INTERNAL_SERVER_ERROR, PLAYER_NOT_FOUND, PLAYER_ROLE_NOT_VALID, PLAYER_SAVE_ERROR, PLAYER_UPDATE_ERROR } from "../../../core/utilities/messages.utility";
...
export class PlayerService implements IPlayerService {
    ...
    public save = async (credentialId: number, createPlayerRequest: CreatePlayerRequest): Promise<PlayerResponse> => {
        ...
        const onboardingStatus = this.resolvePlayerOnboardingStatus(savedPlayer, false);
        await this.credentialRepository.updateOnboardingStatus(credential.id, onboardingStatus);
    ...
    public update = async (updatePlayerRequest: UpdatePlayerRequest): Promise<PlayerResponse> => {
        ...
        const hasActiveTeam = await this.playerTeamRepository.existsActiveTeamForPlayer(updatedPlayer.id);
        const onboardingStatus = this.resolvePlayerOnboardingStatus(updatedPlayer, hasActiveTeam);

        const credentialId = await this.findCredentialIdOrThrow(updatedPlayer.id);
        await this.credentialRepository.updateOnboardingStatus(credentialId, onboardingStatus);
    ...
    private updateBmi = (validatedRequest: UpdatePlayerRequest, player: Player): number | undefined => {
        const finalWeight = validatedRequest.weight !== undefined && validatedRequest.weight !== null
            ? validatedRequest.weight
            : player.weight;

        const finalHeight = validatedRequest.height !== undefined && validatedRequest.height !== null
            ? validatedRequest.height
            : player.height;
    ...
    private isPlayerProfileComplete = (player: Player): boolean => {
        return !!(
            player.birthDate &&
            player.height !== undefined && player.height !== null &&
            player.weight !== undefined && player.weight !== null &&
            player.bmi !== undefined && player.bmi !== null &&
            player.highJump !== undefined && player.highJump !== null &&
            player.rightUnipodalJump !== undefined && player.rightUnipodalJump !== null &&
            player.leftUnipodalJump !== undefined && player.leftUnipodalJump !== null &&
            player.bipodalJump !== undefined && player.bipodalJump !== null &&
            player.thirtyMetersTime !== undefined && player.thirtyMetersTime !== null &&
            player.thousandMetersTime !== undefined && player.thousandMetersTime !== null &&
            player.position
        );
    };

    private resolvePlayerOnboardingStatus = (player: Player, hasActiveTeam: boolean): OnboardingStatus => {
        if (!this.isPlayerProfileComplete(player)) return OnboardingStatus.PROFILE_INCOMPLETE;
        if (!hasActiveTeam) return OnboardingStatus.TEAM_PENDING;
