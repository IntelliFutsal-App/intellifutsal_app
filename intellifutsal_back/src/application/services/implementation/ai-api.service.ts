...
import { AiApiAnalyzeResponse, AiApiFullRecommendationsResponse, AiApiPhysicalResponse, AiApiPositionResponse, AiApiTeamAnalyzeResponse, AiApiTeamPhysicalResponse, AiApiTeamPositionsResponse, CreateClusterRequest, CreatePlayerClusterRequest, PlayerPhysicalResultsResponse, PlayerPositionResultsResponse } from '../../../domain/interfaces';
import { IAiApiService } from "../ai-api.service.interface";
import * as dotenv from "dotenv";
import { BadRequestException } from '../../../domain/exceptions';
import { AI_SERVICE_ANALYTICS_PREDICTION_ERROR, AI_SERVICE_PHYSICAL_PREDICTION_ERROR, AI_SERVICE_PLAYER_WITHOUT_COMPLETE_DATA_PREDICTION_ERROR, AI_SERVICE_POSITION_PREDICTION_ERROR, AI_SERVICE_RECOMMENDATIONS_PREDICTION_ERROR, AI_SERVICE_TEAM_PHYSICAL_PREDICTION_ERROR, AI_SERVICE_TEAM_PLAYERS_WITHOUT_COMPLETE_DATA_PREDICTION_ERROR, AI_SERVICE_TEAM_POSITION_PREDICTION_ERROR, AI_SERVICE_TEAM_WITHOUT_PLAYERS_PREDICTION_ERROR } from '../../../core/utilities/messages.utility';
...
export class AiApiService implements IAiApiService {
    ...
    public predictPosition = async (playerId: number): Promise<AiApiPositionResponse> => {
        ...
        this.validatePlayerDataForAi(player);
    ...
    public analyzeTeamPrediction = async (teamId: number): Promise<AiApiTeamAnalyzeResponse> => {
        ...
        this.validatePlayersDataForAi(players);
    ...
    private validatePlayerDataForAi = (player: any): void => {
        const missingFields: string[] = [];

        if (player.age === undefined || player.age === null) missingFields.push("age");
        if (player.weight === undefined || player.weight === null) missingFields.push("weight");
        if (player.height === undefined || player.height === null) missingFields.push("height");
        if (player.bmi === undefined || player.bmi === null) missingFields.push("bmi");
        if (player.highJump === undefined || player.highJump === null) missingFields.push("highJump");
        if (player.rightUnipodalJump === undefined || player.rightUnipodalJump === null) missingFields.push("rightUnipodalJump");
        if (player.leftUnipodalJump === undefined || player.leftUnipodalJump === null) missingFields.push("leftUnipodalJump");
        if (player.bipodalJump === undefined || player.bipodalJump === null) missingFields.push("bipodalJump");
        if (player.thirtyMetersTime === undefined || player.thirtyMetersTime === null) missingFields.push("thirtyMetersTime");
        if (player.thousandMetersTime === undefined || player.thousandMetersTime === null) missingFields.push("thousandMetersTime");

        if (missingFields.length > 0) {
            throw new BadRequestException(`${ AI_SERVICE_PLAYER_WITHOUT_COMPLETE_DATA_PREDICTION_ERROR }${ missingFields.join(", ") }`);
        }
    };

    private validatePlayersDataForAi = (players: any[]): void => {
        if (!players || players.length === 0) throw new BadRequestException(AI_SERVICE_TEAM_WITHOUT_PLAYERS_PREDICTION_ERROR);
        ...
        players.forEach((player) => {
            ...
            if (missingFields.length > 0) {
                invalidPlayers.push(`${ player.firstName ?? "Jugador" } ${ player.lastName ?? "" }`.trim());
            }
        });

        if (invalidPlayers.length > 0) {
            throw new BadRequestException(`${ AI_SERVICE_TEAM_PLAYERS_WITHOUT_COMPLETE_DATA_PREDICTION_ERROR }${ invalidPlayers.join(", ") }`);
        }
    };
}