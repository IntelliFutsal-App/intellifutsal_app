...
export const useJoinRequests = (activeTeamId?: number | null) => {
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const data = await joinRequestService.findPendingByTeam(activeTeamId);
    ...
    const reject = useCallback(
        async (id: number, reviewComment: string) => {
            try {
                setActing(id, true);
                const updated = await joinRequestService.reject(id, { reviewComment });
                setRequests((prev) => prev.map((r) => (r.id === id ? updated : r)));
